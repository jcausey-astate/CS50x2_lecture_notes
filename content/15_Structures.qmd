...
* Core Guidelines: [<https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#S-enum>]{.small-65}
...
- Initialize with C++11's **_uniform intializer syntax_**:

```cpp
Circle anotherCircle{2, 3, 3.8};
```
...
```cpp
cout << sizeof(anotherCircle);  // using an instance
// ... or ...
cout << sizeof(Circle);         // using the type
```

::: {.aside}
Legacy C++ code may use the following syntax to initialize a structure:
```cpp
Circle yetAnotherCircle = {3, 2, 2.4}; // old style, deprecated
```
:::
...
```
\
Here, _inline member initialization_ is used to provide initial values for `age`.
...
::: {.aside}
**C++ version warning:**\
If you specify initializers, you will not be able to use _uniform initializer syntax_ with your new type in C++11, [but in C++14 and afterward, it is OK.]{.green}
:::
