...
Delete by copy first locates the largest value in the left subtree OR the smallest value in the right subtree (just like _delete by merge_)--let's call that value `tmp`.  Then it simply copies the value from `tmp` into the target, then deletes node `tmp`.
